    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                s = ('%10s' * 2 + '%10.4g' * 7) % (
                    '%g/%g' % (epoch, epochs - 1), mem, *mloss, targets.shape[0], imgs.shape[-1], curr_lr)
        ...
        if rank in [-1, 0]:
            ...
            if tb_writer:
                ...
                for x, tag in zip(list(mloss[:-1]) + list(results) + [curr_lr], tags):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr-scheduler', type=str, default='OneCycle',
                        help='choose learning rate scheduler: Plateau | OneCycle | CosineAnnealing')
